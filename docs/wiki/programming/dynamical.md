---
sidebar_position: 14
sidebar_label: Динамический код
---
[Назад: Служебные локации](service_locations)
...
Гораздо проще написать одну универсальную локацию-функцию (см. раздел ["Пользовательские функции и процедуры"](organizing)), передавать в неё название массива и выводить одним и тем же циклом на печать любой массив.
...
Мы с вами прекрасно умеем работать со строками. Если не умеем, то быстренько читаем раздел ["Строки"](strings/index) и учимся.
...
* Ещё `DYNAMIC` очень удобен для разного рода [отладчиков](../utilits/debugger), поскольку позволяет выполнять код QSP, введённый в строку ввода безо всяких танцев с бубном.
...
[Вперёд: Всплывающее меню](../design/menu)
