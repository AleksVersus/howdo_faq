...
sidebar_position: 2
---
[Назад: Порядок работы интерпретатора](..\principle.md)
...
*  Оно не должно совпадать с ключевыми словами (cм. [список ключевых слов](..\..\keywords.md)).
...
Более подробное описание оператора `KILLVAR` находится в статье про [массивы](..\arrays.md).

## Проверка существования переменной

В **QSP** нет специальной команды для проверки существования переменных, однако в силу особенностей движка можно использовать функцию получения размера [массива](..\arrays.md) `ARRSIZE`. Если функция вернёт единицу **1**, значит переменная существует; если вернёт **0**, значит переменная не существует.
...
О функции `ARRSIZE` так же можно подробнее почитать в разделе ["Массивы"](..\arrays.md).
...
[Вперёд: Выражения](..\expressions.md)
