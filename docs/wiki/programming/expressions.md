...
Вот список операций для плееров **QSP** версии 5.8.0 в порядке повышения приоритета (от меньшего к большему):
...
*  `[$выр1] & [$выр2]` - операция объединения строковых выражений. Подробнее см. [Строки](../strings/index.md). Не путайте с символом перечисления команд. Примеры:
    ``` qsp
...
*  `[$выр1] + [$выр2]` - операция объединения строковых выражений. Подробнее см. [Строки](../strings/index.md). Примеры:
    ``` qsp
    ! в данном случае на экран будет выведена строка "2627"
        "26" + "27"
...
Помимо обыкновенной [операции присваивания](../variables.md) в **QSP** существуют так же совмещённые математические операции с присваиванием.
...
[Вперёд: Программный код](../coding.md)
