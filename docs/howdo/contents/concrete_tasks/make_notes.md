...
! выполяем пока счётчик не достиг размера массива note
    ! выводим запись
    pl $note[i]
end
```
...
На [локации-обработчике выделения предмета](../items/onobjsel.md) добавляем такой код:
...
Проверить работу данного кода можно на [работающем примере](assets/make_notes.qsp).
